...
public class BookingService {
    ...
    public String bookFlight(FlightBookingRequest request) {
        ...
        booking.setBookingDate(LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")));
        booking.setStatus("PENDING"); // Instead of "CONFIRMED"
        booking.setPaymentVerified(false);
        ...
        public Booking getBookingByReference(String reference) {
            Optional<Booking> optionalBooking = bookingRepository.findByBookingReference(reference);
            return optionalBooking.orElseThrow(() -> new RuntimeException("Booking not found"));
        }
        
        @Transactional
        public Booking updatePaymentStatus(String reference, boolean verified) {
            Booking booking = getBookingByReference(reference);
            booking.setPaymentVerified(verified);
            booking.setStatus(verified ? "PAID" : "PENDING");
            return bookingRepository.save(booking); // .save() = merge
        }
